...
Clone the repo:
```bash
git clone https://github.com/YOUR_USERNAME/ghostwriter-skills.git
```
...
More samples = better profiles. Three 500-word samples beats one 1500-word sample.
Keep the genre consistent. Don't mix your tweets with your whitepapers in the same analysis.
Check the confidence score. Anything above 0.8 is solid. Below 0.6, feed it more samples.
Iterate. Analyze, apply, compare to the original, adjust. The blend feature is great for fine-tuning.
Name your profiles well. `blog-voice-casual` is more useful than `my-voice-2` six months later.
Requirements
